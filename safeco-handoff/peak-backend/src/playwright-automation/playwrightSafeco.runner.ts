...
async function closeSafecoModal(page: Page): Promise<void> {
  for (let attempt = 0; attempt < 4; attempt++) {
    ...
    await page.waitForTimeout(90).catch(() => undefined);
  }
}
...
async function clickContinue(page: Page): Promise<void> {
  ...
  // Fallback: DOM-level click for variants where controls are not detected as visible by Playwright.
...
async function advanceUntilAnyVisible(
  ...
  for (let i = 0; i < attempts; i++) {
    ...
    const attachedAfterContinue = await waitForAnyAttached(page, selectors, 3000);
    if (attachedAfterContinue) return attachedAfterContinue;
    const visibleAfterContinue = await waitForAnyVisible(page, selectors, 2500);
...
async function advanceSafecoToSummaryPrint(page: Page): Promise<void> {
  ...
  for (let i = 0; i < 14; i++) {
    ...
    if (stageMarker && summaryStageSelectors.includes(stageMarker)) {
      const latePrint = await waitForAnyVisible(page, [printSelector], 8000);
...
function extractPdfSourceUrlsFromViewerUrl(urlText: string): string[] {
  ...
  try {
    ...
    const candidateParams = ["src", "file", "url"];
...
function extractPdfUrlsFromHtml(baseUrl: string, html: string): string[] {
  const urls = new Set<string>();
  const pushMaybe = (raw: string | undefined): void => {
    if (!raw) return;
    const trimmed = raw.trim().replace(/&amp;/g, "&");
    if (!trimmed) return;
    try {
      const abs = new URL(trimmed, baseUrl).toString();
      if (/pdf|pdfguid|displaypdf|printview/i.test(abs)) {
        urls.add(abs);
      }
    } catch {
...
async function generateSafecoPdf(context: BrowserContext, quotePage: Page, jobId?: string): Promise<string | undefined> {
  ...
  // Last resort: use current page URL (or viewer source URL) and GET raw bytes.
...
async function gotoSafecoPolicyInfo(context: BrowserContext, page: Page, timeoutMs: number): Promise<Page> {
  ...
  for (let attempt = 0; attempt < 4; attempt++) {
    ...
    try {
      ...
      await currentPage.waitForTimeout(250 + attempt * 250).catch(() => undefined);
      currentPage = pickActivePage(context, currentPage);
    }
  }
